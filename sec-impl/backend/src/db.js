...
const DB_USER = process.env.MYSQL_USER || 'sec_impl_user';
const DB_PASSWORD = process.env.MYSQL_PASSWORD || 'sec_impl_pass';
const DB_NAME = process.env.MYSQL_DATABASE || 'juxin_sec_impl';
...
const seedDefaultSlaRules = async () => {
  for (const item of DEFAULT_SLA_RULES) {
    await run(
      `INSERT IGNORE INTO sec_impl_sla_rules
...
const seedDefaultTemplateRules = async () => {
  for (const item of DEFAULT_TEMPLATE_RULES) {
    await run(
      `INSERT IGNORE INTO sec_impl_templates
       (product_code, product_name, enabled)
       VALUES (?, ?, 1)`,
      [item.product_code, item.product_name]
    );
    await run(
      `INSERT IGNORE INTO sec_impl_template_stage_rules
...
const createSchema = async () => {
  await run(`CREATE TABLE IF NOT EXISTS sec_impl_projects (
  ...
  await run(`CREATE TABLE IF NOT EXISTS sec_impl_stage_records (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    ...
    CONSTRAINT fk_stage_job FOREIGN KEY (job_id) REFERENCES sec_impl_projects(id) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);

  await run(`CREATE TABLE IF NOT EXISTS sec_impl_operation_logs (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    job_id BIGINT NULL,
  ...
  await run(`CREATE TABLE IF NOT EXISTS sec_impl_attachments (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    job_id BIGINT NOT NULL,
    ...
    CONSTRAINT fk_attachment_job FOREIGN KEY (job_id) REFERENCES sec_impl_projects(id) ON DELETE CASCADE,
    CONSTRAINT fk_attachment_stage_record FOREIGN KEY (stage_record_id) REFERENCES sec_impl_stage_records(id) ON DELETE SET NULL
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);

  await run(`CREATE TABLE IF NOT EXISTS sec_impl_sla_rules (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
  ...
  await run(`CREATE TABLE IF NOT EXISTS sec_impl_sla_reminders (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    job_id BIGINT NOT NULL,
    stage_code VARCHAR(32) NOT NULL,
    ...
    CONSTRAINT fk_sla_reminder_job FOREIGN KEY (job_id) REFERENCES sec_impl_projects(id) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);

  await run(`CREATE TABLE IF NOT EXISTS sec_impl_templates (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
  ...
  await run(`CREATE TABLE IF NOT EXISTS sec_impl_template_stage_rules (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
  ...
  await run(`CREATE TABLE IF NOT EXISTS sec_impl_deliverables (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    job_id BIGINT NOT NULL,
    stage_code VARCHAR(32) NOT NULL,
    ...
    CONSTRAINT fk_deliverable_job FOREIGN KEY (job_id) REFERENCES sec_impl_projects(id) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);

  await run(`CREATE TABLE IF NOT EXISTS sec_impl_user_customer_scope (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
  ...
  await addColumnIfMissing('sec_impl_stage_records', 'stage_payload', 'stage_payload LONGTEXT NULL');
  await addColumnIfMissing('sec_impl_projects', 'product_type', "product_type VARCHAR(64) NOT NULL DEFAULT ''");
  await addColumnIfMissing('sec_impl_operation_logs', 'chain_prev_hash', 'chain_prev_hash VARCHAR(128) NULL');
  await addColumnIfMissing('sec_impl_operation_logs', 'chain_hash', 'chain_hash VARCHAR(128) NULL');
  await addColumnIfMissing('sec_impl_operation_logs', 'chain_version', "chain_version VARCHAR(16) NOT NULL DEFAULT 'v1'");
  await addIndexIfMissing('sec_impl_projects', 'idx_jobs_status_updated', 'status, updated_at');
  await addIndexIfMissing('sec_impl_projects', 'idx_jobs_product_type', 'product_type');
  await addIndexIfMissing('sec_impl_operation_logs', 'idx_op_created', 'created_at');
  await addIndexIfMissing('sec_impl_operation_logs', 'idx_op_user_created', 'username, created_at');
  await addIndexIfMissing('sec_impl_operation_logs', 'idx_op_chain_hash', 'chain_hash');
  await addIndexIfMissing('sec_impl_attachments', 'idx_attachment_job_stage', 'job_id, stage_code');
  await addIndexIfMissing('sec_impl_sla_reminders', 'idx_sla_reminder_job_stage_time', 'job_id, stage_code, created_at');
  await addIndexIfMissing('sec_impl_sla_reminders', 'idx_sla_reminder_created', 'created_at');
  await addIndexIfMissing('sec_impl_templates', 'idx_template_enabled', 'enabled');
  await addIndexIfMissing('sec_impl_template_stage_rules', 'idx_template_stage_enabled', 'stage_code, enabled');
  await addIndexIfMissing('sec_impl_deliverables', 'idx_deliverable_stage_done', 'stage_code, done_flag');
  await addIndexIfMissing('sec_impl_user_customer_scope', 'idx_scope_customer', 'customer_name');
...
const initDb = async () => {
  ...
  await waitForDb(pool, 'sec_impl database');
  await createSchema();
};
