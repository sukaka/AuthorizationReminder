...
const actionAllowedRoles = {
  assess: ['admin', 'sysadmin'],
  implement: ['admin', 'sysadmin'],
  tune: ['admin', 'sysadmin'],
  trial: ['admin', 'sysadmin'],
  accept: ['admin', 'sysadmin'],
  handover: ['admin', 'sysadmin'],
  close: ['admin', 'sysadmin'],
}
...
function App() {
  ...
  const onLogout = async () => {
    ...
    setToken('')
    setUser(null)
    window.location.href = buildPortalEntryUrl('sec-impl')
  }
  ...
  const refreshJobs = async () => {
    ...
    const result = await apiRequest(`/api/sec-impl/projects?${params.toString()}`, { withMeta: true })
  ...
  const refreshDetail = async (id = selectedJobId) => {
    ...
    try {
      const data = await apiRequest(`/api/sec-impl/projects/${targetId}`)
      setDetail(data)
  ...
  const refreshDashboard = async (overrides = {}) => {
    ...
    try {
      ...
      const data = await apiRequest(`/api/sec-impl/dashboard/summary?${params.toString()}`)
      setDashboard(data)
    } finally {
  ...
  const refreshAuditLogs = async () => {
    ...
    try {
      ...
      const result = await apiRequest(`/api/sec-impl/logs?${params.toString()}`, { withMeta: true })
      const rows = Array.isArray(result?.data) ? result.data : []
      setAuditLogs(rows)
  ...
  const onSaveSlaRules = async () => {
    ...
    try {
      ...
      await apiRequest('/api/sec-impl/sla/rules', {
        method: 'PUT',
        body: { rules },
      })
  ...
  const onRunSlaNow = async () => {
    ...
    try {
      setBusy(true)
      const result = await apiRequest('/api/sec-impl/sla/run', {
        method: 'POST',
        body: { max_scan: 300 },
      })
  ...
  const onBatchDownloadTemplate = async () => {
    try {
      setBusy(true)
      const response = await fetch(`${API_BASE}/api/sec-impl/templates/projects-import.xlsx`, {
        method: 'GET',
        credentials: 'include',
      })
      if (!response.ok) {
      ...
      downloadBlob(blob, 'sec-impl-import-template.xlsx')
      showSuccess('模板下载成功')
    } catch (err) {
      showError(err.message)
    } finally {
  ...
  const onBatchExportJobs = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/sec-impl/reports/projects.xlsx?${params.toString()}`, {
        method: 'GET',
        credentials: 'include',
      })
      if (!response.ok) {
      ...
      const blob = await response.blob()
      downloadBlob(blob, `sec-impl-jobs-${Date.now()}.xlsx`)
      showSuccess('批量导出成功')
    } catch (err) {
      showError(err.message)
    } finally {
  ...
  const onBatchImportJobs = async () => {
    ...
    try {
      ...
      const result = await apiRequest('/api/sec-impl/import/projects.xlsx', {
        method: 'POST',
        formData: true,
        body: formData,
      })
  ...
  const onBatchAdvanceStage = async () => {
    ...
    try {
      setBusy(true)
      const result = await apiRequest('/api/sec-impl/projects/batch/stage', {
        method: 'POST',
        body: {
  ...
  const onVerifyAuditChain = async () => {
    try {
      ...
      const result = await apiRequest(`/api/sec-impl/audit/verify?${params.toString()}`)
  ...
  const onCreateJob = async (e) => {
    ...
    try {
      setBusy(true)
      const created = await apiRequest('/api/sec-impl/projects', {
        method: 'POST',
        body: createForm,
      })
      showSuccess('实施单创建成功')
      setCreateForm({ project_code: '', product_type: '', customer_name: '', sales_order_no: '', inbound_tracking_no: '', remark: '' })
  ...
  const onAdvanceStage = async () => {
    ...
    try {
      ...
      await apiRequest(`/api/sec-impl/projects/${detail.id}/stages/${nextAction}`, {
        method: 'POST',
        body: payload,
      })
      showSuccess('阶段推进成功')
  ...
  const onRework = async () => {
    ...
    try {
      setBusy(true)
      await apiRequest(`/api/sec-impl/projects/${detail.id}/rework`, {
        method: 'POST',
        body: reworkForm,
      })
  ...
  const onUploadAttachment = async () => {
    ...
    try {
      ...
      await apiRequest(`/api/sec-impl/projects/${detail.id}/attachments`, {
        method: 'POST',
        formData: true,
        body: formData,
      })
  ...
  const onDownloadAttachment = async (item) => {
    try {
      const blob = await apiRequest(`/api/sec-impl/attachments/${item.id}/download`, { expectBlob: true })
  ...
  const onDeleteAttachment = async (item) => {
    ...
    openConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          setBusy(true)
          await apiRequest(`/api/sec-impl/attachments/${item.id}`, { method: 'DELETE' })
          showSuccess('附件删除成功')
          await refreshDashboard()
          await refreshDetail()
        } catch (err) {
  ...
  const onExportAudit = async () => {
    try {
      ...
      if (auditFilter.username) params.set('username', auditFilter.username)
      if (auditFilter.keyword) params.set('keyword', auditFilter.keyword)
      const response = await fetch(`${API_BASE}/api/sec-impl/reports/audit.csv?${params.toString()}`, {
        method: 'GET',
        credentials: 'include',
      })
      if (!response.ok) {
      ...
      const blob = await response.blob()
      downloadBlob(blob, `sec-impl-audit-${Date.now()}.csv`)
  ...
  const onExportDashboard = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/sec-impl/reports/dashboard.csv?${params.toString()}`, {
        method: 'GET',
        credentials: 'include',
      })
      if (!response.ok) {
      ...
      const blob = await response.blob()
      downloadBlob(blob, `sec-impl-dashboard-${Date.now()}.csv`)
      ...
      <main className="login-shell">
        <section className="login-card">
          <h1>
            ...
            <span className="brand-blue">实施记录系统</span>
          </h1>
          <p className="sub">使用统一登录进入系统。</p>
          <div className="toolbar">
            <button className="btn btn-primary" onClick={() => (window.location.href = buildPortalEntryUrl('sec-impl'))}>
              前往统一登录
            </button>
            <button className="btn" onClick={() => (window.location.href = buildPortalSwitchUrl('sec-impl'))}>
              切换其他系统
            </button>
          </div>
        </section>
    ...
    <div className="shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-actions">
          <button className="ghost" onClick={() => (window.location.href = buildPortalSwitchUrl('sec-impl'))}>切换系统</button>
      ...
      <main className="content">
        <header className="hero">
          <div>
            ...
            <h1>安全产品实施全流程追踪</h1>
            <div className="sub">流程：评估 → 实施部署 → 联调优化 → 试运行 → 验收确认 → 运维移交 → 归档关闭</div>
          </div>
          <div className="toolbar">
            ...
            <div className="panel-body">
              {!dashboard ? (
                ...
                <>
                  ...
                  <div className="panel-subsection" style={{ marginTop: 14 }}>
                    <strong>超时实施单（{Number(dashboard?.overdue_days || 0)} 天未更新）</strong>
                      ...
                      <table className="table">
                        ...
                        <tbody>
                          ...
                          {(Array.isArray(dashboard?.overdue_jobs) ? dashboard.overdue_jobs : []).length === 0 ? (
                            <tr><td colSpan={6} className="muted">暂无超时实施单</td></tr>
                          ) : null}
                        </tbody>
                      </table>
                    </div>
          ...
          <section className="panel">
            ...
            <div className="panel-body">
              ...
              <div className="panel-subsection" style={{ marginTop: 12 }}>
                ...
                <div className="toolbar" style={{ marginTop: 10 }}>
                  <button className="btn btn-primary" onClick={onBatchExportJobs} disabled={busy}>导出实施单 Excel</button>
                </div>
              </div>
              ...
              <div className="panel-subsection" style={{ marginTop: 12 }}>
                ...
                <div className="grid" style={{ marginTop: 8 }}>
                  ...
                  <div className="field" style={{ gridColumn: '1 / -1' }}>
                    <label>实施单 ID 列表（逗号/空格/换行分隔）</label>
                    <textarea
        ...
        {activeMenu === 'jobs' ? (
          <>
            ...
            {detailModalOpen ? (
              ...
              >
                <section
                  className={`floating-modal sec-impl-detail-modal ${detailModalDragging ? 'dragging' : ''}`}
                  ref={detailModalRef}
                  style={{
                ...
                >
                  <header className="floating-modal-header" onPointerDown={onStartDetailModalDrag}>
                    <div>
                      <h3>实施详情</h3>
                      <div className="muted">实施单号：{detailMatchesSelection ? detail?.job_no || '-' : '-'} | 拖动标题栏可移动</div>
                    </div>
                  ...
                  <div className="floating-modal-body">
                    {!selectedJobId ? (
                      <div className="muted">请先从列表选择一条实施单</div>
                    ) : detailLoading || !detailMatchesSelection ? (
                      <div className="muted">正在加载实施详情...</div>
                    ) : !detail ? (
                      <div className="muted">未找到实施详情</div>
                    ) : (
                      <>
                        <div className="grid">
                          <div className="field"><label>实施单号</label><input value={detail.job_no || '-'} readOnly /></div>
                          ...
                          <div className="field"><label>实施工单号</label><input value={detail.inbound_tracking_no || '-'} readOnly /></div>
              ...
              <div className="grid">
                ...
                <div className="field" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <input
                    ...
                    placeholder="支持实施单号/项目编码/客户/动作/说明"
                  />
                </div>
              </div>
