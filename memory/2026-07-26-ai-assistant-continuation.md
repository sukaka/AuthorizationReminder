...
- 下一步：
  ...
  4. 更新计划与发布记录，升级到 `5.10.0`，只提交本任务文件并推送分支和标签。

## 2026-07-26 继续进度（5.10.0 候选完成）
...
- 修复原有版本契约遗漏：`src-tauri/Cargo.lock` 的主包版本曾停留在 5.8.0，先补齐到 5.9.0 后，再通过原子同步脚本统一升级到 5.10.0。
- 已新增 `docs/releases/5.10.0.md`，明确这是本地候选版本，尚未替代预发布和生产连续观测。
- 剩余操作：检查差异、只暂存本任务文件、提交、打 `ai-assistant-v5.10.0` 标签并推送 `codex/ai-assistant-5.0`。
