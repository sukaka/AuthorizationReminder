...
## 2026-07-26 继续进度（统一登录先选系统文案澄清）

- 桌面端启动页已补充更明确的 3 步说明：先连接服务、再统一登录、最后选择系统。
- Web 端普通登录入口与未登录重定向文案已同步，不再暗示会直接进入某个固定系统。
- 统一登录成功后的提示文案已覆盖：
  - 桌面端启动页；
  - Web 登录过渡页；
  - auth 统一门户提示；
...
- 定向验证已通过：
  - 桌面端 Vitest：31/31；
  - `git diff --check`：通过。
- 本轮按“功能优化”升级版本：`5.13.0 -> 5.14.0`，并只提交本次登录/选择系统相关文件，不带入软著与 SCA 未完成文件。
