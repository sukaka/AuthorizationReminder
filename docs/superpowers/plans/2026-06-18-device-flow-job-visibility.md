...
**Goal:** Ensure only `admin` can view every device-flow job, while other users can view jobs they created or jobs for which they were permanently designated as the second signer, and show every active user in the second-signer selector.

**Architecture:** Add a small pure visibility module that builds one SQL predicate from the authenticated actor. Reuse that predicate in list/aggregate queries and a centralized Express access middleware for job-specific routes, so frontend hiding is never the security boundary. For device-flow only, expose every active user through the authenticated system directory and remove the erroneous frontend role filter.
...
- [x] Return every active user from `/api/auth/system-users?system=device-flow` and build options excluding only the current user.
