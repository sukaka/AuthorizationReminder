...
- 关系型业务数据主要位于一个 MySQL 实例的多个 Schema，现役业务 Schema 使用独立最小权限账号。
...
| 中间件 | 使用方 | 当前设计 |
| --- | --- | --- |
| MySQL 8 | Auth、Reminder、Delivery、CMDB、Inventory、Device Flow、FAQ、Tender、Train Exam、Prompt Center、Big Screen | 单实例、多 Schema、独立运行账号 |
...
设备流转系统使用独立 Schema 和独立运行账号 `device_flow_user`，避免与其他业务系统共享数据库口令。
...
| 数据库 | 引擎 | 主要所有者 | 备注 |
| --- | --- | --- | --- |
| `juxin_reminder` | MySQL | Reminder、Auth | `reminder_user` 与 `auth_user` 分离 |
| `juxin_delivery` | MySQL | Delivery | 现役交付域独立库 |
| `cmdb` | MySQL | CMDB | 独立账号 `cmdb_user` |
| `juxin_inventory` | MySQL | Inventory | 独立账号 `inventory_user` |
| `juxin_device_flow` | MySQL | Device Flow | 独立账号 `device_flow_user` |
...
| `juxin_big_screen` | MySQL | Big Screen | 独立账号 `big_screen_user`，只保存大屏自身配置与运行数据 |
...
- 11 个现役业务后端统一暴露 `/api/health`、`/api/ready`、`/api/version`、`/api/build`、`/api/metrics`，发布验证脚本应按系统清单逐项检查。
...
当前 Compose 已清除生产配置中的固定示例密钥和弱默认值；`.env.example` 仅保留变量结构。生产部署必须由密钥管理系统或受控环境变量注入强密码、签名密钥和第三方令牌。
...
SCA 已对 PostgreSQL 和 Redis 配置 Compose 健康检查；其他业务的依赖启动条件并不完全等价于业务就绪，运维脚本必须执行统一的应用级健康、就绪、版本、构建信息和指标接口验证。
...
### 10.4 OWASP Top 10:2025 控制映射

平台安全设计以 OWASP Top 10:2025 为统一控制口径，并将控制点落到认证、授权、配置、依赖、数据、审计和运行治理中。

| OWASP 项 | 平台防护设计 |
| --- | --- |
| A01:2025 Broken Access Control | 统一门户会话、Auth introspection、系统级角色与服务端鉴权；写接口必须校验用户、系统、角色和对象范围。 |
| A02:2025 Security Misconfiguration | README、旧拓扑文档和 Compose 统一 11 个现役系统口径；生产配置不得保留默认密钥、示例密码和不必要入口。 |
| A03:2025 Software Supply Chain Failures | SCA 平台持续扫描 SBOM、依赖漏洞和许可证风险；发布门禁要求高危依赖有修复或风险接受结论。 |
| A04:2025 Cryptographic Failures | JWT、审计签名、配置加密、Webhook 和第三方令牌由环境注入强密钥；不得提交真实密钥或弱默认值。 |
| A05:2025 Injection | 后端使用参数化查询、ORM 或受控 SQL；导入、搜索、分页和批量接口限制输入类型、长度和数量。 |
| A06:2025 Insecure Design | 高层设计明确系统边界、数据所有权、兼容资产退役路线和恢复演练要求，避免跨系统隐式耦合。 |
| A07:2025 Authentication Failures | 统一 Cookie、会话撤销、过期校验、登录失败审计和生产 HTTPS/Secure Cookie 策略。 |
| A08:2025 Software or Data Integrity Failures | 审计签名链、版本化配置、文档回调签名、镜像构建记录和发布回退点共同保证变更可追溯。 |
| A09:2025 Security Logging and Alerting Failures | 统一审计中心、业务操作日志、SCA 持续风险监测和健康/就绪接口纳入日常检查。 |
| A10:2025 Mishandling of Exceptional Conditions | API 不返回堆栈、SQL、密钥或内部路径；超时、依赖失败和异常状态需要稳定错误码与可观测告警。 |

本次治理已落地四项基础动作：统一 11 个现役系统口径，清除生产配置中的固定示例密钥和弱默认值，每个业务 Schema 使用独立最小权限账号，统一健康、就绪、版本和构建信息接口。

### 10.5 当前安全风险

- 历史兼容资产仍需要保留迁移和回溯能力，需避免继续扩展新入口。
- 共享 MySQL 实例仍扩大数据库故障影响面，生产需依赖备份恢复和网络隔离降低风险。
- 生产 `.env` 若未按规范注入强密钥，会影响 JWT、审计签名和配置加密。
- 依赖漏洞需要通过 SCA 扫描、修复或风险接受闭环持续治理。
...
- 11 个现役业务后端统一应用级接口：`/api/health`、`/api/ready`、`/api/version`、`/api/build`、`/api/metrics`。
- 11 个现役业务后端统一生成或继承 `X-Request-Id`，并在结构化访问日志中输出 `request_id`、路径、状态码和 `duration_ms`。
- Docker Compose 容器状态与结构化日志。
- 业务操作日志和统一审计中心。
- SCA 任务、扫描和持续风险监测记录。
- Big Screen 来源健康接口。
...
当前已按“服务健康、业务连续性、资源容量、链路定位”四层落地基础可观测口径：

| 指标层级 | 重点指标 | 用途 |
| --- | --- | --- |
| 服务黄金指标 | HTTP 请求量、错误数、并发数、平均耗时、最大耗时、状态码分布 | 已由各业务后端 `/api/metrics` 提供进程级快照，用于发布回归和容量告警 |
| 认证与权限 | Auth 登录成功率、失败原因、会话数、鉴权延迟、越权拦截次数 | 识别登录异常、权限配置错误和认证中心性能瓶颈 |
| 数据与存储 | 数据库连接池使用率、慢查询、容量、备份状态、文件卷容量、增长率、孤儿文件数量 | 提前发现容量风险、备份缺口和数据访问性能问题 |
| 异步任务 | Worker 队列长度、任务耗时、失败率、重试次数、死信数量 | 发现 SCA 扫描、报告生成、外部同步等后台任务积压 |
| 外部依赖 | 第三方服务成功率、超时率、熔断状态、降级次数、最近恢复时间 | 评估物流、漏洞源、AI、OCR、对象存储等依赖对业务的影响 |
| 安全运营 | 登录失败、鉴权失败、敏感配置变更、审计签名异常、SCA 高危新增数 | 支撑 OWASP A09 安全日志与告警治理 |

当前已完成各业务 API 的结构化访问日志和 `request_id` 贯穿基础能力。后续应接入统一日志平台、指标采集器和分布式追踪，把网关、Auth、业务 API、Worker、数据库访问、外部依赖调用和审计记录串成完整链路，做到一次异常可以从日志、指标、链路和审计四个视角定位。
...
- 已更新 README 和旧拓扑文档，统一 11 个现役系统口径。
- 已清除生产配置中的固定示例密钥和弱默认值。
- 已为每个业务 Schema 使用独立最小权限账号。
- 已统一健康、就绪、版本、构建信息和基础指标接口。
- 已统一 11 个现役业务后端的 `X-Request-Id` 和结构化访问日志。
- 建立数据库与文件卷配套恢复演练。
