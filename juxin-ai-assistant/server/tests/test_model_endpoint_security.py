...
def _agent_settings(**overrides):
    values = {
        "auth_dev_bypass": False,
        "agent_http_endpoint_enabled": True,
        "agent_http_allowed_hosts": "agent.example.com",
    }
    values.update(overrides)
    return SimpleNamespace(**values)


def test_agent_http_endpoint_is_disabled_by_default():
    with pytest.raises(HTTPException) as exc_info:
        validate_agent_http_endpoint(
            "https://agent.example.com/invoke",
            _agent_settings(agent_http_endpoint_enabled=False),
        )

    assert exc_info.value.status_code == 403
    assert exc_info.value.detail == "AGENT_HTTP_ENDPOINT_DISABLED"


def test_agent_http_endpoint_reuses_https_allowlist_and_private_dns_policy(monkeypatch):
    with pytest.raises(HTTPException) as exc_info:
        validate_agent_http_endpoint("http://agent.example.com/invoke", _agent_settings())
    assert exc_info.value.detail == "AGENT_HTTP_ENDPOINT_INVALID"

    monkeypatch.setattr(
        "app.model_endpoint_security.socket.getaddrinfo",
        lambda *args, **kwargs: [(2, 1, 6, "", ("192.168.1.5", 443))],
    )
    with pytest.raises(HTTPException) as exc_info:
        validate_agent_http_endpoint(
            "https://agent.example.com/invoke",
    ...
    assert exc_info.value.detail == "AGENT_HTTP_ENDPOINT_PRIVATE_NETWORK"
