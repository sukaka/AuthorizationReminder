...
def _validated_document_archive(data: bytes, *, file_kind: str) -> ZipFile:
    ...
    for entry in entries:
        if entry.flag_bits & 0x1:
            archive.close()
            raise HTTPException(status_code=422, detail=f"{file_kind} 文件包含加密压缩条目")
        total_size += entry.file_size
        if entry.file_size > MAX_ARCHIVE_ENTRY_UNCOMPRESSED_BYTES:
            archive.close()
            raise HTTPException(status_code=422, detail=f"{file_kind} 文件包含过大的压缩条目")
        if (
    ...
    if total_size > MAX_ARCHIVE_UNCOMPRESSED_BYTES:
        archive.close()
        raise HTTPException(status_code=422, detail=f"{file_kind} 文件解压后体积过大")
    return archive


def _validate_extracted_blocks(blocks: list[ParsedBlock]) -> list[ParsedBlock]:
    if len(blocks) > MAX_EXTRACTED_BLOCKS:
        raise HTTPException(status_code=422, detail="文件提取出的内容块过多")
    if sum(len(block.text) for block in blocks) > MAX_EXTRACTED_TEXT_CHARS:
        raise HTTPException(status_code=422, detail="文件提取出的文本内容过大")
    return blocks
...
def _parse_pdf_blocks(data: bytes) -> list[ParsedBlock]:
    ...
    if len(reader.pages) > MAX_PDF_PAGES:
        raise HTTPException(status_code=422, detail="PDF 页数过多")
