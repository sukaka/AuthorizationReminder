...
validate_https_url() {
  ...
  VALIDATED_URL="$raw" EXACT_ORIGIN="$exact_origin" node -e '
    try {
  ...
  ' || {
    echo "$variable_name 必须是合法、无凭据的 HTTPS $value_kind" >&2
    exit 1
  }
}

if [[ -n "$DEFAULT_SERVER_ORIGIN" ]]; then
  validate_https_url "AI_ASSISTANT_DEFAULT_SERVER_ORIGIN" "$DEFAULT_SERVER_ORIGIN" true
...
case "$UPDATER_ENABLED" in
  true)
    validate_https_url "AI_UPDATER_URL" "$UPDATER_URL" false
