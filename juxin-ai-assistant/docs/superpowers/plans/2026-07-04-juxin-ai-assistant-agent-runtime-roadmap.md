...
- [ ] 公司知识检索工具：只查管理员上传或审核通过的正式资料。
- [ ] 我的资料检索工具：只查当前用户个人资料。
- [ ] 当前附件检索工具：只查本次会话上传附件。
- [ ] 文件解析工具：统一解析 txt、md、docx、xlsx、pptx、pdf。
- [ ] Word 导出工具：支持“仅本次生成内容”和“聚信格式 Word”。
- [ ] 引用来源校验工具：只保留实际被回答使用的来源。
- [ ] 资料入库申请工具：用户确认后提交管理员审核。
- [ ] 管理员审核工具：通过、驳回、改分类、启用正式知识检索。
- [ ] 工具调用日志工具：记录每次工具调用状态、耗时、来源数量、错误码。

### 2. P1 应该工具化

- [ ] Web 搜索工具：用于联网查找公开资料。
- [ ] 网页抓取工具：抓取网页内容、生成摘要、等待用户确认保存。
- [ ] 个人记忆工具：保存“我的偏好”，按 user_id 隔离。
- [ ] 文档模板选择工具：根据任务类型选择输出结构和 Word 模板。
- [ ] 文档结构校验工具：检查输出是否符合聚信文档规范。
- [ ] 任务模式识别工具：识别售前、交付、安全运维、风险评估、行政人力等任务。
- [ ] 历史任务工具：读取历史任务、恢复上下文、重新生成。
- [ ] 用户反馈工具：收集有帮助、不准确、格式不合适等反馈。
...
- [ ] 梳理现有聊天、资料库、Word 导出、模型配置、管理员审核链路。
- [ ] 固化现有关键接口清单。
- [ ] 确认“普通用户”和“管理员”权限边界。
- [ ] 确认当前版本号，按功能优化规则规划下一版为第二位升级。
- [ ] 建立最小回归用例：普通聊天、查公司知识、查我的资料、上传附件、导出 Word、管理员审核。
...
- [ ] 定义 `ToolInput`、`ToolResult`、`ToolError`、`ToolContext`。
- [ ] 定义 `BaseTool`，统一 `name`、`description`、`permission`、`timeout_seconds`、`execute()`。
- [ ] 定义 `ToolRegistry`，支持注册、查询、执行、禁用、版本标记和权限检查。
- [ ] 包装现有公司知识检索、我的资料检索、当前附件检索。
- [ ] 包装现有 Word 导出。
- [ ] 包装现有资料入库申请和管理员审核。
- [ ] 增加工具调用日志，避免只靠散落业务日志排查。
...
- [ ] 将 ContextBuilder 明确拆成 Gather、Select、Structure、Compress 四步。
- [ ] Gather：收集系统提示、公司画像、助手模式、用户问题、最近历史、我的偏好、检索证据、工具结果。
- [ ] Select：按权限、相关性、新近性、资料等级选择上下文。
- [ ] Structure：按 Role、Task、Evidence、Context、Output 拼装消息。
- [ ] Compress：长对话超过阈值后生成摘要，只保留关键决策、未完成事项、已确认事实。
- [ ] 回答完成后，根据实际回答内容过滤引用来源。
- [ ] Word 导出使用同一套“实际引用来源”结果。
- [ ] 没有检索命中时，不把无关文件放到参考来源。
...
- [ ] 增加 PDF 文本解析。
- [ ] 统一 txt、md、docx、xlsx、pptx、pdf 的块结构：文件名、页码/章节、sheet/slide、chunk_index。
- [ ] 上传后自动生成摘要、建议分类、建议文档类型。
- [ ] 普通用户默认保存到“我的资料”。
- [ ] 申请进入公司知识库时，必须二次确认。
- [ ] 管理员审核通过后，才允许进入公司知识检索。
- [ ] 审核不通过时，资料仍可保留在用户“我的资料”中。
- [ ] 支持上传文件改名，并同步影响前端展示和引用名称。
...
- [ ] 聊天页资料来源显示为：公司知识、我的资料、当前附件、联网查找。
- [ ] 上传后显示紧凑附件条，不占用输入框。
- [ ] 文件保存选项：仅本次使用、保存到我的资料、申请加入公司知识库。
- [ ] 我的资料页支持：上传、搜索、分类、改名、问文档、根据资料生成、申请入库。
- [ ] 历史任务支持：查看、继续、导出、删除。

**管理员前端：**

- [ ] 审核队列：待审核资料、提交人、摘要、建议分类、片段预览。
- [ ] 审核动作：通过、驳回、修改分类、启用/停用公司知识检索。
- [ ] 分类管理：一级分类、二级分类、创建、编辑、删除、排序。
- [ ] 审计记录：谁上传、谁审核、何时入库、是否被引用。
