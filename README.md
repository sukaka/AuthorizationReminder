...
### 2.2.1 新服务器首启（一键）
```bash
git clone -b codex/5.8.0 https://github.com/sukaka/AuthorizationReminder.git /root/AuthorizationReminder-codex-5.8.0
cd /root/AuthorizationReminder-codex-5.8.0
...
> 说明：`bootstrap-full-server.sh` 默认把仓库同步到 `/root/AuthorizationReminder-codex-5.8.0`，并使用分支 `codex/5.8.0`。如需覆盖，可设置 `BOOTSTRAP_REPO_DIR`、`BOOTSTRAP_BRANCH`、`BOOTSTRAP_REPO_URL`。`ALIYUN_MIRROR_URL`、`AUTH_BUILTIN_ACCOUNT_DEFAULT_PASSWORD`、`PUBLIC_HOST` 为必填项。`PUBLIC_HOST` 只写主机名/IP，不要带 `http://` 或端口。
>
