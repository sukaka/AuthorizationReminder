...
### 2.2.1 新服务器首启（一键）
```bash
git clone -b codex/4.3.3 https://github.com/sukaka/AuthorizationReminder.git /root/AuthorizationReminder-codex-4.3.3
cd /root/AuthorizationReminder-codex-4.3.3
...
> 说明：`bootstrap-full-server.sh` 默认把仓库同步到 `/root/AuthorizationReminder-codex-4.3.3`，并使用分支 `codex/4.3.3`。如需覆盖，可设置 `BOOTSTRAP_REPO_DIR`、`BOOTSTRAP_BRANCH`、`BOOTSTRAP_REPO_URL`。`ALIYUN_MIRROR_URL`、`AUTH_BUILTIN_ACCOUNT_DEFAULT_PASSWORD`、`PUBLIC_HOST` 为必填项。`PUBLIC_HOST` 只写主机名/IP，不要带 `http://` 或端口。
>
