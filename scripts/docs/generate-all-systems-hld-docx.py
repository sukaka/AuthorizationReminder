...
def draw_overview() -> Path:
    ...
    draw.text(
        (125, 1200),
        "说明：主图保留高层直观性；完整端口、数据卷、启动依赖与健康检查在第 9 章容器编排设计中展开。",
        font=pil_font(23),
        fill=MUTED,
    )
...
def draw_data_topology() -> Path:
    image, draw = canvas("平台数据拓扑", "单 MySQL 实例多业务库；软件成分分析独立使用 PostgreSQL / Redis；文件卷单独备份")
...
def draw_deployment() -> Path:
    image, draw = canvas("当前容器编排部署关系", "单一容器网络，前端静态站点、业务接口与共享中间件分层运行")
    round_rect(draw, (70, 185, 2130, 1190), "#F7FAFC", "#9BB3C5", width=4)
    draw.text((100, 205), "容器编排网络", font=pil_font(32, True), fill=NAVY)
