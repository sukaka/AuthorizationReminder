...
echo "[7/17] 测试"
request_json POST "/api/device-flow/jobs/$JOB_ID/stages/test" '{"remark":"测试通过","stage_payload":{"boot_test":"PASS","network_test":"PASS","stress_test":"PASS","test_result":"PASS"}}' >/dev/null

echo "[8/17] 审核"
request_json POST "/api/device-flow/jobs/$JOB_ID/stages/approve" '{"remark":"审核通过","stage_payload":{"approve_result":"PASS","approve_note":"符合交付标准"}}' >/dev/null
...
echo "[13/17] 校验日志接口"
LOGS="$(request_json GET "/api/device-flow/logs?page=1&limit=10&keyword=SN-SMOKE-001")"
...
echo "[16/17] 校验审计验签接口"
VERIFY_RESP="$(request_json GET "/api/device-flow/audit/verify?limit=100")"
