...
test('system config menu exposes upload and OpenAI settings', () => {
  ...
  assert.match(appSource, /api_base_url/)
  assert.match(appSource, /model_name/)
  assert.match(appSource, /\/api\/sca\/system-config/)
  assert.match(appSource, /testModelConnection/)
  assert.match(appSource, />测试</)
...
test('uploads use runtime config and nginx allows configured request bodies', () => {
  assert.match(apiSource, /VITE_UPLOAD_CHUNK_SIZE_MB/)
...
test('project task status refreshes while asynchronous jobs are active', () => {
  ...
  assert.match(appSource, /loadProjectActivity/)
  assert.match(appSource, /Promise\.allSettled/)
  assert.match(appSource, /import\('\.\/composables\/projectDataLoader\.js'\)/)
  assert.match(appSource, /const completed = await loadProjectActivity\(\)[\s\S]{0,100}if \(completed\) await loadProjectDetails\(\)/)
